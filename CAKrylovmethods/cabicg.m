...
function results = cabicg(A, b, s, x0, maxits, tol, basis_type)
...
while its < maxits
    ...
    P = computeBasis(A,p(:,s*k+1),s,alp,bet,gam);
    
    %Compute Krylov basis with starting vector pt
    Pt = computeBasis(A',pt(:,s*k+1),s,alp,bet,gam);
    
    %Compute Krylov basis with starting vector r
    R = computeBasis(A,r(:,s*k+1),s-1,alp,bet,gam);
    
    %Compute Krylov basis with starting vector rt
    Rt = computeBasis(A',rt(:,s*k+1),s-1,alp,bet,gam);
    
    %Construct block "basis change" matrix
    Tt  = sparse([T, zeros(s+1,s+1); zeros(s,s+1), T(1:s,1:(s-1)),zeros(s,1)]);
    
    % Initialize BICG coordinate vectors for current outer loop
    p_c = [[1;zeros(2*s,1)],zeros(2*s+1,s)];
    pt_c = [[1;zeros(2*s,1)],zeros(2*s+1,s)];
    r_c = [[zeros(s+1,1);1;zeros(s-1,1)],zeros(2*s+1,s)];
    rt_c = [[zeros(s+1,1);1;zeros(s-1,1)],zeros(2*s+1,s)];
    x_c = zeros(2*s+1,s+1);
    ...
    for j = 1:s
        ...
        alpha(its) = (rt_c(:,j)'*G*r_c(:,j)) / (pt_c(:,j)'*G*Tt*p_c(:,j));
        ...
        %Update r coordinate vectors
        r_c(:,j+1) = r_c(:,j) - alpha(its)*Tt*p_c(:,j);
        rt_c(:,j+1) = rt_c(:,j) - alpha(its)*Tt*pt_c(:,j);
        
        %Perform basis change to compute r and rt vectors in standard basis (note we wouldn't need to do this
        %in the inner loop in practice)
        r(:,s*k+j+1)  = [P,R]*r_c(:,j+1);
        rt(:,s*k+j+1)  = [Pt,Rt]*rt_c(:,j+1);
        
        %Compute scalar beta using Gram matrix for inner products
        beta(its) = (rt_c(:,j+1)'*G*r_c(:,j+1)) / (rt_c(:,j)'*G*r_c(:,j));
        
        %Update p coordinate vectors
        p_c(:,j+1) = r_c(:,j+1) + beta(its)*p_c(:,j);
        pt_c(:,j+1) = rt_c(:,j+1) + beta(its)*pt_c(:,j);
        
        %Perform basis change to compute p and pt vectors in standard basis (note we wouldn't need to do this
        %in the inner loop in practice)
        p(:,s*k+j+1)  = [P,R]*p_c(:,j+1);
        pt(:,s*k+j+1)  = [Pt,Rt]*pt_c(:,j+1);
